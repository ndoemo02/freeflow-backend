// Direct database setup for restaurant access
...
async function setupUserRestaurants() {
  ...
  try {
    ...
    // 2. Create a test user ID (we'll use a fixed UUID for ndoemo02@gmail.com)
    const testUserId = '66051f90-6486-4ce3-b771-f51d3d39a8e9'; // This should be the actual user ID
    
    console.log('👤 Using test user ID:', testUserId);
    
    // 3. Update existing restaurants to be owned by the user
    if (restaurants && restaurants.length > 0) {
      console.log('🔗 Connecting existing restaurants to user...');
      ...
      for (const restaurant of restaurants) {
        const { error: updateError } = await supabase
          ...
          .update({ 
            owner_id: testUserId,
            owner: testUserId  // Update both fields for compatibility
          })
          .eq('id', restaurant.id);
        ...
        if (updateError) {
          ...
          console.log(`✅ Connected ${restaurant.name} to user ${testUserId}`);
        }
      }
    } else {
    ...
    console.log('📝 Note: Make sure you are logged in as ndoemo02@gmail.com in the frontend.');
